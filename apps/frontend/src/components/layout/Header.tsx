  ...
  return (
    <header className="fixed top-0 left-[260px] right-0 h-16 bg-white border-b border-gray-100 z-30 flex items-center justify-between px-6">
      <div className="flex items-center gap-4">
        ...
        {user?.role === 'ADMIN' && (
          <div className="relative">
            ...
            >
              ...
              <span className="text-sm font-medium text-[#064E3B]">{activeTenant?.name ?? 'Workspace'}</span>
                ...
                <div className="absolute left-0 top-10 z-50 bg-white rounded-xl shadow-lg border border-gray-100 py-1 min-w-[220px]">
                  {(tenants as any[]).map((t: any) => (
                    <div
                      ...
                      className={cn(
                        ...
                        activeTenant?.id === t.id ? 'bg-[#10B981]/10 text-[#10B981] font-medium' : 'text-gray-600 hover:bg-gray-50',
                      )}
                    >
                      <button
                  ...
                  <div className="border-t border-gray-100 mt-1 pt-1">
                    <button
                      ...
                      className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-[#10B981] hover:bg-gray-50"
                    >
        ...
        <div>
          <h1 className="text-lg font-heading font-bold text-[#064E3B] leading-tight">{title}</h1>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => navigate('/settings')}
          className="p-2 rounded-xl text-gray-400 hover:bg-gray-50 hover:text-[#10B981] transition-colors">
          <Settings className="w-5 h-5" />
        </button>

        <div className="flex items-center gap-2 ml-2">
          <div className="w-8 h-8 rounded-full bg-[#10B981]/10 flex items-center justify-center text-[#10B981] text-sm font-semibold">
            {user?.email?.[0]?.toUpperCase() ?? 'U'}
          </div>
          <span className="text-sm font-medium text-gray-600 hidden md:block">
            {user?.email ?? 'user@voxai.app'}
          </span>
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-base font-semibold text-[#064E3B]">Novo Workspace</h2>
            <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Nome do workspace"
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#10B981]" />
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div className={cn(
                        ...
                        newNiche === option.key ? 'border-[#10B981]' : 'border-gray-300',
                      )}>
                        {newNiche === option.key && <div className="w-2 h-2 rounded-full bg-[#10B981]" />}
                      </div>
                      <span className="text-sm font-medium text-[#064E3B]">{option.label}</span>
                    </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={() => createTenant.mutate()} disabled={!newName.trim() || createTenant.isPending}
                className="px-4 py-2 text-sm bg-[#10B981] text-white rounded-lg hover:bg-[#059669] disabled:opacity-40">
                Criar
              </button>
            </div>
          </div>
